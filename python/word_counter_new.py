...
class NumCounterNew:
    ...
    def save_file_from_dic(self, filename, data):
    ...
    def save_file_from_lists(self, filename, words, counts):
      df = pd.DataFrame({"words": words, "counts": counts})
      df.sort_values(by=["counts"], ascending=False)
      df = df.reset_index()
    ...
    def add_text_to_counts(self, load_filename, save_filename, text):
      ...
      for word in text:
        ...
        if word in dict_:
          dict_[word] += 1
      self.save_file_from_dic(save_filename, dict_)
    ...
    def remove_one_counts(self, load_filename, save_filename, counts_to_remove=1):
        # get dataframe
        os.chdir("D:/Users/flopp/Documents/VSCode/Python/NLP")
        ...
        # save as df
        self.save_file_from_lists(save_filename, words, counts)
        return df
...
new_counter.remove_one_counts("new_word_list.txt", "new_word_list_edit.txt", 10)
